...
namespace Bookstore.Tests.Controllers
{
    public class ImportControllerTests
    {
        ...
        {
            // Arrange
            var imports = new List<WarehouseImportDto> { new() { ImportId = "imp1", UserId = "user1", ImportDate = DateTime.UtcNow } };
        ...
        [Fact]
        public async Task CreateImport_ReturnsBadRequest_WhenExceptionThrown()
        {
            // Arrange
            var dto = new WarehouseImportDto { ImportId = "imp1", UserId = "user1" };
            _importServiceMock.Setup(s => s.CreateImportAsync(dto)).ThrowsAsync(new ArgumentException("Danh sách sách nhập không được để trống"));
            Func<WarehouseImportDto, IImportService, Task<IResult>> createImport = async (dto, service) =>
            {
                try
                {
            ...
            Assert.Equal("Danh sách sách nhập không được để trống", ((dynamic)badRequestResult.Value).message);
        }
    }
}